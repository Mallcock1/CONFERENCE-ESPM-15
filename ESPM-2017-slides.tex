...
%
%\begin{figure}
%\begin{tikzpicture}[node distance=1cm and 1cm, auto,>=latex', align = flush center, thick, on grid=false, font=\small]
%    % We need to set at bounding box first. Otherwise the diagram
%    % will change position for each frame.
%    \path[use as bounding box] (-1,0) rectangle (10,-2);
%    \path[]<1-> node[format] (obs) {
%    		\tb{Observations}};
%    \path[]<1-> node[below= of obs] (ghost) {};
%    \path[]<1-> node[format, below= of ghost] (phys) {
%    		\tb{Physical} \\ 
%    		\tb{understanding}};
%    	
%    \path[]<2-> node[format, above right= of obs] (sp) {
%    		\tb{Spatial} \\ 
%    		\tb{parameters}};
%    \path[]<2-> node[format, right= of obs] (eq p) {
%    		\tb{Equilibrium} \\ 
%    		\tb{parameters}};
%    \path[]<2-> node[format, below right= of obs] (tp) {
%    		\tb{Temporal} \\ 
%    		\tb{parameters}};
%%    \path[]<3-> node[format, below of=tp] (doo) {
%%    		\tb{Temporal} \\ 
%%    		\tb{parameters}};
%
%    \path[]<2-> node[format, below= of tp] (eq m) {
%    					\tb{Equilibrium} \\ 
%    					\tb{models}}
%                   edge[<-] (phys);
